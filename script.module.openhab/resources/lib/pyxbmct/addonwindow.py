...
class _AbstractWindow(object):
    ...
    def placeControl(self, control, row, column, rowspan=1, columnspan=1, pad_x=5, pad_y=5):
        ...
        """
        try:
            control_x = int((self.grid_x + self.tile_width * column) + pad_x)
            control_y = int((self.grid_y + self.tile_height * row) + pad_y)
            control_width = int(self.tile_width * columnspan - 2 * pad_x)
            control_height = int(self.tile_height * rowspan - 2 * pad_y)
